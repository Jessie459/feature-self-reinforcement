...
parser.add_argument("--work_dir", default="work_dir_coco/fsr", type=str)
...
parser.add_argument("--fsr_iters", type=int, default=80000)
...
def train(args):
    ...
    logging.info(f"size of dataset (val): {len(val_dataset)}")
    ...
    for n_iter in range(args.max_iters):
        ...
        if (n_iter + 1) <= 8000:
            loss = cls_loss + w_class * fsr_cls_loss + w_patch * fsr_pat_loss + 0.0 * aff_loss + 0.0 * seg_loss + 0.0 * reg_loss
        elif (n_iter + 1) <= 12000:
            loss = cls_loss + w_class * fsr_cls_loss + w_patch * fsr_pat_loss + 0.0 * aff_loss + 0.1 * seg_loss + 0.05 * reg_loss
        else:
            loss = cls_loss + w_class * fsr_cls_loss + w_patch * fsr_pat_loss + 0.2 * aff_loss + 0.1 * seg_loss + 0.05 * reg_loss
