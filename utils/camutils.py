...
def ignore_img_box(label, img_box, ignore_index):
    ...
    for idx, coord in enumerate(img_box):
        pseudo_label[idx, coord[0] : coord[1], coord[2] : coord[3]] = label[
...
def crop_from_roi_neg(images, roi_mask=None, crop_num=8, crop_size=96):
    ...
    for i1 in range(b):
        ...
        if roi_index.shape[0] < crop_num:
            roi_index = (
                roi_mask[i1, margin : (h - margin), margin : (w - margin)] >= 0
            ).nonzero()  ## if NULL then random crop
...
def multi_scale_cam(model, inputs, scales):
    ...
    _cam, _aux_cam = model(inputs_cat, cam_only=True)

    _cam = F.interpolate(_cam, size=(h, w), mode="bilinear", align_corners=False)
    _cam = torch.max(_cam[:b, ...], _cam[b:, ...].flip(-1))

    _aux_cam = F.interpolate(_aux_cam, size=(h, w), mode="bilinear", align_corners=False)
    _aux_cam = torch.max(_aux_cam[:b, ...], _aux_cam[b:, ...].flip(-1))

    cam_list = [F.relu(_cam)]
    aux_cam_list = [F.relu(_aux_cam)]
    ...
    for s in scales:
        if s != 1.0:
            ...
            aux_cam_list.append(F.relu(_aux_cam))
    ...
    aux_cam = torch.sum(torch.stack(aux_cam_list, dim=0), dim=0)
    aux_cam = aux_cam + F.adaptive_max_pool2d(-aux_cam, (1, 1))
    aux_cam = aux_cam / (F.adaptive_max_pool2d(aux_cam, (1, 1)) + 1e-5)
